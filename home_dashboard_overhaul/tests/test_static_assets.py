...
class CorrectedStaticAssetTests(unittest.TestCase):
    ...
    def test_calendar_state_hierarchy_has_distinct_visual_responsibilities(self) -> None:
        ...
        self.assertIn("box-shadow: inset 0 0 0 2px var(--calendar-today-ring)", today_rule)
        ...
        self.assertIn("box-shadow: 0 -1px 0 var(--ui-text-primary)", due_rule)
        event_rule = self.css.split(".hdo-event-marker {", 1)[1].split("}", 1)[0]
        self.assertIn("background: var(--status-event-fill)", event_rule)
        self.assertIn("outline: 1px solid var(--calendar-event-halo)", event_rule)
    ...
    def test_shared_shell_uses_release_container_breakpoints_and_geometry(self) -> None:
        self.assertIn("width: min(1120px, calc(100% - 40px))", self.css)
        self.assertIn("max-width: 1120px", self.css)
        self.assertIn("margin: 24px auto 0", self.css)
        ...
        self.assertIn("@container hdo-dashboard (min-width: 1000px)", self.css)
        ...
        progress_chip_rule = self.css.split(
            ".hdo-progress-heading-value {", 1
        )[1].split("}", 1)[0]
        self.assertIn("max-width: 100%", progress_chip_rule)
        self.assertIn("white-space: nowrap", progress_chip_rule)
        progress_rule = self.css.split(".hdo-progress-track {", 1)[1].split("}", 1)[0]
        self.assertIn("min-block-size: 14px", progress_rule)
        self.assertEqual(self.html.count("data-hdo-progress-label"), 0)
        self.assertEqual(self.html.count("data-hdo-progress-value"), 1)
    ...
    def test_live_canvas_is_transparent_while_cards_and_preview_remain_themed(self) -> None:
        ...
        scroll_clearance = self.js.split(
            "function applyDocumentScrollClearance(root) {", 1
        )[1].split("function mount()", 1)[0]
        ...
        self.assertIn("document.scrollingElement", scroll_clearance)
        self.assertIn("visibleBottomActionContainer(root)", scroll_clearance)
        self.assertIn("var measured = 60", scroll_clearance)
        self.assertIn("var clearance = footerHeight + 24", scroll_clearance)
        self.assertIn('setProperty("padding-block-end", clearance + "px")', scroll_clearance)
        self.assertIn('setProperty("scroll-padding-block-end", clearance + "px")', scroll_clearance)
        self.assertIn("hdoFooterClearanceSource", scroll_clearance)
        self.assertIn("new global.ResizeObserver(update)", scroll_clearance)
        self.assertNotIn('setProperty("overflow-x"', scroll_clearance)

    def test_year_grid_is_a_stable_53_week_tree_and_month_is_six_weeks(self) -> None:
    ...
    def test_refresh_failure_is_one_timestamped_full_width_banner(self) -> None:
        self.assertEqual(
            self.js.count('warning.className = "hdo-data-warning hdo-refresh-warning"'),
            1,
        )
        self.assertIn('if (root.querySelector(".hdo-refresh-warning")) return;', self.js)
        self.assertIn("root.dataset.hdoLastUpdatedAt", self.js)
        self.assertIn("Refresh failed. Showing data last updated at", self.js)
        self.assertIn("last_updated_at", self.html)
