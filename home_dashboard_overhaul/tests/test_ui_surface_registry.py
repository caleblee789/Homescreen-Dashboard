...
class UiSurfaceRegistryTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.registry = json.loads(
            (ROOT / "qa" / "ui-surface-registry_1_8_7.json").read_text(encoding="utf-8")
        )
        cls.manifest = json.loads(
            (ROOT / "qa" / "calendar_surface_manifest_1_8_7.json").read_text(encoding="utf-8")
        )
    ...
    def test_registry_has_one_current_entry_per_authoritative_surface(self) -> None:
        ...
        self.assertEqual(self.registry["release"], "1.8.7")
