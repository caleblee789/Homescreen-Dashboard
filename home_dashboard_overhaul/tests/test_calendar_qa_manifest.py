...
class NativeRefinementQaContractTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.manifest = json.loads(
            (qa / "calendar_surface_manifest_1_8_2.json").read_text(encoding="utf-8")
        )
        cls.matrix = json.loads(
            (qa / "visual_regression_matrix_1_8_2.json").read_text(encoding="utf-8")
        )
        cls.capture = json.loads(
            (qa / "capture_evidence_manifest_1_8_2.json").read_text(encoding="utf-8")
        )
        cls.registry = json.loads(
            (qa / "ui-surface-registry_1_8_2.json").read_text(encoding="utf-8")
        )

    def test_manifest_is_the_authoritative_1_8_2_native_contract(self) -> None:
        self.assertEqual(self.manifest["schema_version"], 8)
        self.assertEqual(self.manifest["release"], "1.8.2")
        self.assertEqual(
            self.manifest["contract"],
            "final-ui-correction-native-100-percent-2026-08-23",
        )
        self.assertEqual(
        ...
        self.assertEqual(architecture["minimum_container_width"], 940)
        self.assertEqual(architecture["dashboard_maximum_width"], 1320)
        self.assertEqual(architecture["minimum_page_side_margin"], 16)
        self.assertEqual(architecture["top_spacing"], 22)
        self.assertEqual(architecture["bottom_safe_area"], 72)
    ...
    def test_primary_matrix_is_the_exact_sixteen_case_cartesian_product(self) -> None:
        ...
        self.assertEqual(self.matrix["release"], "1.8.2")
    ...
    def test_capture_manifest_binds_every_primary_and_required_coverage_tag(self) -> None:
        ...
        self.assertEqual(self.capture["native_frame_count"], {"initial": 47, "restart": 1, "total": 48})
        self.assertEqual(self.capture["contact_sheet_output"]["detail_sheet_count"], 13)
