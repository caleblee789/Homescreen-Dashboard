#!/usr/bin/env python3
"""Assemble immutable Home Screen Dashboard 1.8.5 release evidence."""
...
EXPECTED_RESTART = 2
EXPECTED_TOTAL = 97
DEFAULT_CANDIDATE = SOURCE_ROOT / "dist" / "home-dashboard-overhaul-1.8.5.ankiaddon"
DEFAULT_OUTPUT = SOURCE_ROOT / "qa" / "release-evidence-1.8.5-2026-08-24"
...
def expected_capture_ids() -> list[str]:
    matrix = read_json(SOURCE_ROOT / "qa" / "visual_regression_matrix_1_8_5.json")
    contract = read_json(SOURCE_ROOT / "qa" / "capture_evidence_manifest_1_8_5.json")
    ...
    require(len(ids) == EXPECTED_TOTAL and len(set(ids)) == EXPECTED_TOTAL, "capture contract does not derive 97 unique IDs")
    return ids
...
def make_capture_sheet(
    ...
    draw.text((24, 58), "Home Screen Dashboard 1.8.5 · native exact-package evidence", font=font(16), fill="#cbd5e1")
...
def report_sheet(output: Path, isolation: Mapping[str, Any], archive: Mapping[str, Any]) -> dict[str, Any]:
    ...
    lines = [
        ...
        "Restart persistence: production Year + Settings clean state + event name sort + window size",
    ...
    for line in lines:
        ...
        y += 66
    path = output / "contact-sheets" / "contact-sheet-18-package-and-isolation-reports.png"
    path.parent.mkdir(parents=True, exist_ok=True)
    canvas.save(path, "PNG", optimize=True)
    return {
        "file": path.relative_to(output).as_posix(),
...
def detail_groups() -> list[tuple[str, list[str]]]:
    ...
    return [
        ...
        ("Settings overlay, About bottom, legacy route, restore, and clamp", [contract[index] for index in (6, 15, 20, 21, 22)]),
        ("Controlled restart persistence", expected[95:97]),
    ]
...
def write_readme(output: Path, candidate_hash: str) -> None:
    text = """# Home Screen Dashboard 1.8.5 native release evidence

This immutable evidence set was assembled from the exact reproducible package
`home-dashboard-overhaul-1.8.5.ankiaddon` with SHA-256 `{}`.
...
def assemble(run_root: Path, candidate: Path, output: Path) -> Path:
    ...
    run_output = run_root / "hdo-release-evidence-1.8.5"
    ...
    contract_names = (
        "calendar_surface_manifest_1_8_5.json",
        "ui-surface-registry_1_8_5.json",
        "visual_regression_matrix_1_8_5.json",
        "capture_evidence_manifest_1_8_5.json",
        "runtime_probe_release_1_8_5_manifest.json",
    )
