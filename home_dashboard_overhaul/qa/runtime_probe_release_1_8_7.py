...
def _prepare_settings_case(case: Mapping[str, Any]) -> SettingsDialog:
    ...
    elif special in {"future-off", "future-on"}:
        dialog.calendar_display_disclosure.setChecked(True)
    elif special in {
        "dirty",
        "discard",
        "close-confirmation",
    ...
    return {
        ...
        * max(0, feedback_intersection.height()),
        ...
        "quote_list_bounded": quote_list is None or (
            180 <= quote_list.minimumHeight()
            and quote_list.minimumHeight() <= quote_list.height()
            and quote_list.height() <= quote_list.maximumHeight()
            and quote_list.maximumHeight() <= 520
        ),
...
def _structured_settings_case_matrix() -> list[dict[str, Any]]:
    ...
    page_fixtures: dict[str, tuple[str, dict[str, Any]]] = {
        "dashboard": (
            ...
            {
                "kind": "widget",
                "attribute": "local_data_disclosure",
                "scroll": "bottom",
            },
        ),
        "events": (
...
def _assert_scoped_settings_resets(dialog: SettingsDialog) -> None:
    ...
    def context_state() -> dict[str, Any]:
        ...
        state.update(
            ...
            calendar_disclosure=dialog.calendar_display_disclosure.isChecked(),
            local_data_disclosure=dialog.local_data_disclosure.isChecked(),
        )
        return state
