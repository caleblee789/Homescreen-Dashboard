"""Disposable-only exact-package layout acceptance probe for Home Dashboard 1.6.0.

This helper is intentionally not shipped in the add-on archive.  Copy it into a
fresh sync-disabled QA base as ``addons21/zz_hdo_16_layout_probe/__init__.py``.
...
EVIDENCE = RUN_ROOT / "evidence-layout-1.6.0"
RESULT_PATH = RUN_ROOT / "acceptance-result-layout-1.6.0.json"
PHASE_PATH = RUN_ROOT / "acceptance-phase-layout-1.6.0.json"
SEED_MARKER = RUN_ROOT / "synthetic-layout-fixture-1.6.0.json"
IDENTITY_PATH = RUN_ROOT / "QA_IDENTITY.json"
PACKAGE_ROOT = RUN_ROOT / "addons21" / "home_dashboard_overhaul"
RUNTIME_TEST_PATH = RUN_ROOT / "test_calendar_repository.py"
...
def compile_installed_python() -> bool:
    errors = []
    compiled = 0
    for path in sorted(PACKAGE_ROOT.rglob("*.py")):
        try:
            ...
            errors.append({"path": str(path.relative_to(PACKAGE_ROOT)), "error": str(exc)})
    values = {"compiled_files": compiled, "errors": errors, "passed": not errors}
    record("packaged_python_compilation", values)
    return bool(values["passed"])


def run_runtime_repository_tests() -> bool:
    if not RUNTIME_TEST_PATH.is_file():
        record(
            "runtime_calendar_repository_tests",
            {"passed": False, "error": "Runtime test module was not installed"},
        )
        return False
    stream = io.StringIO()
    spec = importlib.util.spec_from_file_location(
        "hdo_16_runtime_calendar_repository_tests", RUNTIME_TEST_PATH
    )
    if spec is None or spec.loader is None:
        record(
            "runtime_calendar_repository_tests",
            {"passed": False, "error": "Could not load runtime test module"},
        )
        return False
    ...
    record("runtime_calendar_repository_tests", values)
    return bool(values["passed"])
...
var progressValueFor=function(label){
  var stat=progressStats.find(function(item){return item.querySelector('dt').textContent.trim()===label;});
  return stat?stat.querySelector('dd').textContent.trim():'';
};
...
var result={
  ...
  progressMetricCount:progressStats.length,
  progressVisualRows:new Set(progressStats.map(function(item){return Math.round(item.getBoundingClientRect().top);})).size,
  progressLabels:progressStats.map(function(item){return item.querySelector('dt').textContent.trim();}),
  progressValues:{
    percent:progressValueFor('Percent Complete'),
    new:progressValueFor('New remaining'),
    learning:progressValueFor('Learning remaining'),
    review:progressValueFor('Reviews remaining'),
    total:progressValueFor('Total remaining')
  },
  progressState:progressBar&&progressBar.dataset.hdoProgressState,
  progressAria:progressBar&&progressBar.getAttribute('aria-valuetext'),
...
def new_card(label: str, sequence: int) -> int:
    ...
    if len(note.fields) > 1:
        note.fields[1] = "Synthetic Home Dashboard 1.6 exact-package layout fixture"
    note.tags.append("hdo_16_layout_acceptance")
...
def add_tenth_answer() -> None:
    try:
        history_card = int(mw.col.db.scalar(
            "SELECT c.id FROM cards c JOIN notes n ON n.id=c.nid WHERE n.tags LIKE '%hdo_16_layout_acceptance%' AND n.flds LIKE 'History%' ORDER BY c.id LIMIT 1"
        ))
...
def scroll_to_statistics(callback: Callable[[], None]) -> None:
    eval_js(
        "var s=document.querySelector('.hdo-stat-groups');if(s)s.scrollIntoView({block:'start'});return !!s;",
        lambda _raw: callback(),
        450,
...
def prepare_zoom_150() -> None:
    ...
    QTimer.singleShot(
        900,
        lambda: click_view("month", lambda: scroll_to_statistics(capture_zoom_150)),
    )
...
def prepare_zoom_200() -> None:
    ...
    QTimer.singleShot(
        900,
        lambda: click_view("month", lambda: scroll_to_statistics(capture_zoom_200)),
    )
...
def capture_calendar_settings(dialog: Any) -> None:
    ...
    dialog.reject()
    QTimer.singleShot(500, prepare_event_manager)


def prepare_event_manager() -> None:
    mw._home_dashboard_overhaul_controller.open_event_manager()
    QTimer.singleShot(500, lambda: wait_for_event_manager(0))


def wait_for_event_manager(attempt: int) -> None:
    dialog = mw._home_dashboard_overhaul_controller.event_manager_dialog
    if dialog is not None and dialog.isVisible():
        dialog.resize(1180, 760)
        QTimer.singleShot(700, lambda: capture_event_manager(dialog))
        return
    if attempt >= 30:
        fail("event_manager", RuntimeError("Event Manager did not open"))
        return
    QTimer.singleShot(200, lambda: wait_for_event_manager(attempt + 1))


def capture_event_manager(dialog: Any) -> None:
    capture("08b-event-manager", dialog)
    manager_text = " ".join(
        item.text().strip() for item in dialog.findChildren(QWidget)
        if hasattr(item, "text") and callable(item.text) and item.text().strip()
    )
...
def complete_acceptance() -> None:
    ...
    required_progress_labels = [
        ...
        "Reviews remaining", "Total remaining",
    ]
    responsive = [
        metrics.get("04b_month_620x780_stats", {}),
    ...
    remaining = expected9["remaining"]
    workload = 10 + remaining["new"] + remaining["learning"] + remaining["review"]
    expected_percent = (10 * 100 + workload // 2) // workload
    expected_progress_values = {
        "percent": "{}%".format(expected_percent),
        "new": str(remaining["new"]),
        "learning": str(remaining["learning"]),
        "review": str(remaining["review"]),
        "total": str(remaining["new"] + remaining["learning"] + remaining["review"]),
    }
    summary = {
        ...
        "five_progress_entries_responsive_passed": all(
            item.get("progressMetricCount") == 5
            and item.get("progressVisualRows") == 5
            and item.get("progressLabels") == required_progress_labels
            and not item.get("horizontalOverflow")
            and not item.get("groupClipping")
            and item.get("statValuesVisible")
            for item in responsive
        ),
        "eta_owned_only_by_today_passed": all(
            item.get("etaCount") == 1
            and item.get("todayLabels") == required_today_labels
            and "ETA" not in item.get("progressLabels", [])
            for item in metrics.values()
        ),
        "progress_values_match_synthetic_queue_passed": (
            year.get("progressValues") == expected_progress_values
            and year.get("progressValueNow") == str(expected_percent)
        ),
        "progress_accessibility_passed": (
            year.get("progressRole") == "progressbar"
            and not year.get("progressInteractive")
            and all(
                phrase in str(year.get("progressAria", ""))
                for phrase in (
                    "{}% complete".format(expected_percent),
                    "10 completed answers",
                    "{} new remaining".format(remaining["new"]),
                    "{} learning remaining".format(remaining["learning"]),
                    "{} reviews remaining".format(remaining["review"]),
                    "{} total remaining".format(expected_progress_values["total"]),
                )
            )
        ),
        ...
        "event_manager_opened_and_rendered_passed": (
            ...
            and RESULTS.get("event_manager", {}).get("title") == "Event Manager — Home Dashboard"
            and "Event Manager" in RESULTS.get("event_manager", {}).get("text", "")
        ),
        "packaged_python_compilation_passed": bool(
            RESULTS.get("packaged_python_compilation", {}).get("passed")
        ),
        "runtime_calendar_repository_suite_passed": bool(
            RESULTS.get("runtime_calendar_repository_tests", {}).get("passed")
        ),
...
def start() -> None:
    try:
        ...
        if stage == "restart":
            ...
            if not compile_installed_python():
                raise RuntimeError("Packaged Python compilation failed")
            if not run_runtime_repository_tests():
                raise RuntimeError("Runtime calendar repository tests failed")
            seed_collection()
    except Exception as exc:
        fail("start", exc)
