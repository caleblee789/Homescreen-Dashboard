#!/usr/bin/env python3
"""Validate the machine-readable Home Dashboard 1.8.3 UI contract.
...
def validate(root: Path = ROOT) -> List[str]:
    errors: List[str] = []
    manifest = _read(root, "calendar_surface_manifest_1_8_3.json")
    matrix = _read(root, "visual_regression_matrix_1_8_3.json")
    registry = _read(root, "ui-surface-registry_1_8_3.json")
    capture = _read(root, "capture_evidence_manifest_1_8_3.json")

    if manifest.get("release") != "1.8.3" or manifest.get("schema_version") != 8:
        errors.append("surface manifest must describe release 1.8.3 / schema 8")
    if manifest.get("contract") != "final-corrections-native-100-percent-2026-08-23":
...
def main() -> int:
    ...
    print(
        ...
        "(1.8.3 schema 8, 56 native frames, 9 representative widths, no restart waiver)"
    )
    return 0
