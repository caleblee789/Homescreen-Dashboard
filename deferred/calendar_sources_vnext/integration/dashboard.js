(function (global) {
  ...
  function groupEvents(rows) {
    ...
    (Array.isArray(rows) ? rows : []).forEach(function (item) {
      ...
      grouped[item.date].push({
        id: String(item.id || ""),
        occurrenceId: String(item.occurrence_id || item.id || ""),
        date: item.date,
        name: item.name,
        sourceId: String(item.source_id || "local"),
        sourceName: typeof item.source_name === "string" && item.source_name ? item.source_name : "Local",
  ...
  function dayPreview(dateString, todayString, completed, newCardsStudied, due, locale) {
    var parsed = parseDate(dateString);
    var today = parseDate(todayString);
    if (!parsed || !today) return { date: "Invalid date", summary: "" };
    var full = parsed.toLocaleDateString(locale || undefined, {
      weekday: "short", year: "numeric", month: "short", day: "numeric"
    });
    var summary = dayDifference(parsed, today) <= 0
      ? countLabel(completed, "review", "reviews") + " · " +
        countLabel(newCardsStudied, "new card studied", "new cards studied")
      : countLabel(due, "card due", "cards due");
    return { date: full, summary: summary };
  }

  function popoverLabel(dateString, completed, due, events, locale, newCardsStudied) {
    var parsed = parseDate(dateString);
    if (!parsed) return "Invalid date";
    var full = parsed.toLocaleDateString(locale || undefined, {
      weekday: "long", year: "numeric", month: "long", day: "numeric"
    });
    var label = full + ": " + (Number(completed) || 0) + " completed reviews";
    if (newCardsStudied !== undefined) {
      label += ", " + countLabel(newCardsStudied, "new card studied", "new cards studied");
    }
    label += ", " + (Number(due) || 0) + " reviews due";
    if (events && events.length) label += ", " + events.length + (events.length === 1 ? " event" : " events");
    return label;
  }
  ...
  function initCalendar(root) {
    ...
    var rawEvents = Array.isArray(payload.events) ? payload.events.slice() : [];
    var events = groupEvents(rawEvents);
    ...
    var state = {
      ...
      chipCapacity: -1,
      calendarGeneration: Number(payload.calendar_generation) || 0,
      requestSerial: 0,
      pendingRequest: 0,
      loadedRanges: []
    };
    if (parseDate(payload.loaded_start) && parseDate(payload.loaded_end)) {
      state.loadedRanges.push({ start: payload.loaded_start, end: payload.loaded_end });
    }
    ...
    var loadStatus = region.querySelector("[data-hdo-calendar-load-status]");
    var manageCalendarsButton = region.querySelector("[data-hdo-manage-calendars]");
    ...
    function desiredChipCapacity() {
      return state.view === "month" && calendarWidth() >= 720 ? 2 : 0;
    }
    ...
    function showDetails(cell) {
      ...
      if (!parsed) return;
      region.querySelector("[data-hdo-detail-date]").textContent = parsed.toLocaleDateString(undefined, {
        weekday: "long", year: "numeric", month: "long", day: "numeric"
      });
      region.querySelector("[data-hdo-detail-completed]").textContent = String(Number(item.reviews_completed) || 0);
      region.querySelector("[data-hdo-detail-due]").textContent = String(Number(item.reviews_due) || 0);
      ...
      dateEvents.forEach(function (eventItem) {
        ...
        eventName.className = "hdo-date-event-name";
        eventName.textContent = eventItem.name;
        var sourceName = document.createElement("span");
        sourceName.className = "hdo-date-event-source";
        sourceName.textContent = eventItem.sourceName;
      ...
      browseButton.dataset.date = iso;
      if (manageButton) manageButton.dataset.date = iso;
    ...
    function createDay(dateValue, outside, view, rowIndex, columnIndex) {
      ...
      button.addEventListener("keydown", function (event) {
        var amount = { ArrowLeft: -1, ArrowRight: 1, ArrowUp: -7, ArrowDown: 7 }[event.key];
    ...
    function rangeIsLoaded(start, end) {
      return state.loadedRanges.some(function (loaded) {
        return loaded.start <= start && loaded.end >= end;
      });
    }

    function ensureCalendarRange(range) {
      if (!state.loadedRanges.length || root.dataset.hdoPreview === "true") return;
      var start = isoDate(range.displayStart);
      var end = isoDate(addDays(range.displayEnd, 1));
      if (rangeIsLoaded(start, end)) return;
      state.requestSerial += 1;
      state.pendingRequest = state.requestSerial;
      if (loadStatus) loadStatus.textContent = "Loading calendar events…";
      command("calendar_events_range", {
        start: start,
        end: end,
        generation: state.calendarGeneration,
        request_id: state.pendingRequest
      });
    }
    ...
    function render(focusAfter) {
      ...
      for (var cursor = range.displayStart; cursor <= range.displayEnd; cursor = addDays(cursor, 1)) {
        ...
        if (state.view === "year" && outside) {
          ...
          rows[rowIndex - 1].appendChild(createDay(cursor, outside, state.view, rowIndex, columnIndex));
        }
        position += 1;
      }
    ...
    if (manageButton) manageButton.addEventListener("click", function () {
      if (manageButton.dataset.date) command("manage_events", { date: manageButton.dataset.date });
    });
    if (manageCalendarsButton) manageCalendarsButton.addEventListener("click", function () {
      command("manage_events", { section: "calendars" });
    });

    var namespace = global.HDOHomeDashboard = global.HDOHomeDashboard || {};
    namespace.receiveCalendarEvents = function (response) {
      if (!response || Number(response.generation) !== state.calendarGeneration) return;
      if (Number(response.request_id) !== state.pendingRequest) return;
      state.pendingRequest = 0;
      if (response.error) {
        if (loadStatus) loadStatus.textContent = "Calendar events unavailable: " + String(response.error);
        return;
      }
      if (!parseDate(response.start) || !parseDate(response.end) || !Array.isArray(response.events)) return;
      rawEvents = rawEvents.filter(function (item) {
        return !item || typeof item.date !== "string" || item.date < response.start || item.date >= response.end;
      }).concat(response.events);
      events = groupEvents(rawEvents);
      state.loadedRanges.push({ start: response.start, end: response.end });
      if (loadStatus) loadStatus.textContent = "Calendar events updated";
      render(false);
    };
